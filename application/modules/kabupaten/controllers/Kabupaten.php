...
class Kabupaten extends MY_Controller{
	 public function __construct()
  	{
        ...
        $this->susbtitle = 'Kabupaten';//subtitle
    ...
    public function actived_Class($dataactiveclass){
        $activeclass = $this->Kabupaten_m->get_activedclass($dataactiveclass);
        // var_dump($activeclass);
        // $arr_word = $activeclass['linkactive'];
        // var_dump($arr_word);
        if($activeclass['tit_actclass']!=''){
            $arr_actclass = explode('/', $activeclass['tit_actclass']);
            foreach ($arr_actclass as $actclass) {
                $this->session->set_tempdata($actclass,"active",0,1);//class active nanti diteruskan
                // var_dump($actclass);
            }
        }
    }
    public function index()
    {
        ...
        $jumlah_data = $this->Kabupaten_m->jumlah_data();
        ...
        $data['results'] = $this->Kabupaten_m->get_index($config['per_page'],$from);
    ...
    {//cari data
        ...
        if($datacari == ''){
            ...
            $jumlah_data = $this->Kabupaten_m->jumlah_data_cari();
            ...
            if($jumlah_data != 0){
              ...
              $data['numberofpage'] = '';
                ...
                $data['results'] = $this->Kabupaten_m->get_cari($config['per_page'],$from,$datacari);
                $data['content_view'] = strtolower($this->susbtitle).'/'.strtolower($this->susbtitle).'_v';
                $this->template->hygienic_template($data);
        }
    }
    public function add(){
        ...
        $data['datapages'] = $this->Kabupaten_m->get_dropdowndata();
    ...
    public function view($view_id){
        $data['subtitle'] = $this->susbtitle;
        if($this->Kabupaten_m->get_id($view_id)->row_array()){
            $data['results'] = $this->Kabupaten_m->get_id($view_id)->row_array();
            $data['groupid'] = $this->Kabupaten_m->getselect($view_id);
        }
        else{
            redirect('not_found');
        }
    ...
    public function update($update_id){
        if ($this->form_validation->run(strtolower($this->susbtitle).'/update') == FALSE)
        {
            ...
            $this->Kabupaten_m->updated($update_id);
            $this->session->set_flashdata("infoupdate","<script>
                                                     setTimeout(function() {
                                                       toastr.options = {
                                                          'closeButton': true,
    ...
    public function delete($delete_id){
        $this->Kabupaten_m->deleted($delete_id);
        redirect(strtolower($this->susbtitle));
    }
    public function delete_multiple(){
        $this->Kabupaten_m->deleted_multiple();
        redirect(strtolower($this->susbtitle));
    }
    ...
    public function generateReport(){
        ...
        $data['pageresult'] = $this->Kabupaten_m->select_groups();
