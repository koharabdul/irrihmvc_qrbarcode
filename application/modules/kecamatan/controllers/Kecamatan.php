...
class Kecamatan extends MY_Controller{
	 public function __construct()
  	{
        parent::__construct();
        $this->load->model('Kecamatan_m');//model
        $this->session->set_tempdata("masterwilayah","active",0,1);//class active        
        $this->susbtitle = 'Kecamatan';//subtitle
        $this->sumof_perpage = 10;
        $this->Kecamatan_m->jumlah_data();
    }
    public function index()
    {
        $data['subtitle'] = $this->susbtitle;
        $jumlah_data = $this->Kecamatan_m->jumlah_data();
        ...
        $data['results'] = $this->Kecamatan_m->get_index($config['per_page'],$from);
    ...
    {//cari data
        ...
        if($datacari == ''){
            ...
            $jumlah_data = $this->Kecamatan_m->jumlah_data_cari();
            ...
            if($jumlah_data != 0){
              ...
              $data['numberofpage'] = '';
                ...
                $data['results'] = $this->Kecamatan_m->get_cari($config['per_page'],$from,$datacari);
                $data['content_view'] = strtolower($this->susbtitle).'/'.strtolower($this->susbtitle).'_v';
                $this->template->hygienic_template($data);
        }
    }
    public function add(){
        ...
        $data['datapages'] = $this->Kecamatan_m->get_dropdowndata();
    ...
    public function saves(){
        ...
        // $this->Kecamatan_m->input_multiple();       
    ...
    public function view($view_id){
        $data['subtitle'] = $this->susbtitle;
        if($this->Kecamatan_m->get_id($view_id)->row_array()){
            $data['results'] = $this->Kecamatan_m->get_id($view_id)->row_array();
            $data['groupid'] = $this->Kecamatan_m->getselect($view_id);
        }
        else{
            redirect('not_found');
        }
    ...
    public function update($update_id){
        if ($this->form_validation->run(strtolower($this->susbtitle).'/update') == FALSE)
        {
            ...
            $this->Kecamatan_m->updated($update_id);
            $this->session->set_flashdata("infoupdate","<script>
                                                     setTimeout(function() {
                                                       toastr.options = {
                                                          'closeButton': true,
    ...
    public function delete($delete_id){
        $this->Kecamatan_m->deleted($delete_id);
        redirect(strtolower($this->susbtitle));
    }
    public function delete_multiple(){
        $this->Kecamatan_m->deleted_multiple();
        redirect(strtolower($this->susbtitle));
    }
    ...
    public function generateReport(){
        ...
        $data['pageresult'] = $this->Kecamatan_m->select_groups();
