...
class Penduduk_m extends CI_Model{
    ...
    public function show_data($nama_alamat,$nik_or_kk,$rt,$rw,$jns_kelamin,$set_umur,$umur,$smpumur,$sampaiTanggal){
        $this->db->select("*");
        $this->db->select("CONCAT(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),7,2),' ',
                    IF(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)=1,'Januari',
                    IF(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)=2,'Februari',
                    IF(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)=3,'Maret',
                    IF(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)=4,'April',
                    IF(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)=5,'Mei',
                    IF(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)=6,'Juni',
                    IF(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)=7,'Juli',
                    IF(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)=8,'Agustus',
                    IF(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)=9,'September',
                    IF(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)=10,'Oktober',
                    IF(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)=11,'Nopember',
                    IF(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)=12,'Desember',
                    '')))))))))))
                    ),' ',
                LEFT(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),4))createdAt");
        if(($sampaiTanggal==0) or (empty($sampaiTanggal)) or ($sampaiTanggal=='')){
             $this->db->select("CASE
                            WHEN ((MID(DATE_FORMAT(NOW(),'%Y%m%d'),5,2))<(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)))
                            THEN ((LEFT(DATE_FORMAT(NOW(),'%Y%m%d'),4)-(LEFT(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),4))-1))
                            WHEN (MID(DATE_FORMAT(NOW(),'%Y%m%d'),5,2)=MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2))
                            THEN (CASE
                                WHEN (RIGHT(DATE_FORMAT(NOW(),'%Y%m%d'),2)<RIGHT(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),2))
                                THEN ((LEFT(DATE_FORMAT(NOW(),'%Y%m%d'),4)-(LEFT(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),4))-1))
                                ELSE ((LEFT(DATE_FORMAT(NOW(),'%Y%m%d'),4)-(LEFT(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),4)))) END
                                  ) 
                            ELSE (LEFT(DATE_FORMAT(NOW(),'%Y%m%d'),4)-(LEFT(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),4))) END AS age");
        }
        else{
            $this->db->select("CASE
                            WHEN ((MID(DATE_FORMAT('".$sampaiTanggal."','%Y%m%d'),5,2))<(MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2)))
                            THEN ((LEFT(DATE_FORMAT('".$sampaiTanggal."','%Y%m%d'),4)-(LEFT(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),4))-1))
                            WHEN (MID(DATE_FORMAT('".$sampaiTanggal."','%Y%m%d'),5,2)=MID(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),5,2))
                            THEN (CASE
                                WHEN (RIGHT(DATE_FORMAT('".$sampaiTanggal."','%Y%m%d'),2)<RIGHT(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),2))
                                THEN ((LEFT(DATE_FORMAT('".$sampaiTanggal."','%Y%m%d'),4)-(LEFT(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),4))-1))
                                ELSE ((LEFT(DATE_FORMAT('".$sampaiTanggal."','%Y%m%d'),4)-(LEFT(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),4)))) END
                                  ) 
                            ELSE (LEFT(DATE_FORMAT('".$sampaiTanggal."','%Y%m%d'),4)-(LEFT(DATE_FORMAT(b.tgl_lahir,'%Y%m%d'),4))) END AS age");
        }
