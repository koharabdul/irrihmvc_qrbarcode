...
class MY_Session
{
    ...
    {
        log_message('debug', "Custom My_SESSION Class Initialized");
        ...
        if (is_numeric($sess_expiration) && $sess_expiration > 0) {
            if (config_item('sess_regenerate')) {
                if (!isset($_SESSION['_sess:last-generated'])) {
                    ...
                    $expiry_time = $_SESSION['_sess:last-generated'] +
                        $sess_expiration;
                    if (time() >= $expiry_time) {
                        return true;
                    }
                ...
                if (isset($_SESSION['_sess:last-activation'])) {
                    $expiry_time = $_SESSION['_sess:last-activation'] +
                        $sess_expiration;
                    if (time() >= $expiry_time) {
                        $this->destroy();
                        return true;
    ...
    function _flashdata_sweep()
    {
        foreach ($_SESSION as $name => $value) {
            ...
            if (is_array($parts) && count($parts) == 2 &&
                $parts[0] == config_item('sess_flash_key')
            ) {
