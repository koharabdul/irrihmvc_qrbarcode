...
class MY_Model extends CI_Model
{
     ...
     */
    protected $before_create = array('timestamps','userstamps');
    ...
    public function get_by()
    {
        ...
        if ($this->soft_delete && $this->_temporary_with_deleted !== TRUE) {
            $this->_database->where($this->soft_delete_key,
                (bool)$this->_temporary_only_deleted);
        }
    ...
    public function update($primary_value, $data, $skip_validation = FALSE)
    {
        ...
        if ($data !== FALSE) {
            $result = $this->_database->where($this->primary_key,
                $primary_value)
                ->set($data)
                ->update($this->_table);
                                ...
                                $skip_validation = FALSE)
    {
        ...
        if ($data !== FALSE) {
            $result = $this->_database->where_in($this->primary_key,
                $primary_values)
                ->set($data)
                ->update($this->_table);
    ...
    public function relate($row)
    {
        ...
        foreach ($this->belongs_to as $key => $value) {
            ...
            if (in_array($relationship, $this->_with)) {
                ...
                if (is_object($row)) {
                    $row->{$relationship} = $this->{$relationship . '_model'}->get(
                        $row->{$options['primary_key']});
                } else {
                    $row[$relationship] = $this->{$relationship . '_model'}->get(
                        $row[$options['primary_key']]);
                }
        ...
        foreach ($this->has_many as $key => $value) {
            ...
            if (in_array($relationship, $this->_with)) {
                ...
                if (is_object($row)) {
                    $row->{$relationship} = $this->{$relationship . '_model'}->get_many_by(
                        $options['primary_key'], $row->{$this->primary_key});
                } else {
                    $row[$relationship] = $this->{$relationship . '_model'}->get_many_by(
                        $options['primary_key'], $row[$this->primary_key]);
                }
    ...
    public function get_next_id()
    {
        return (int)$this->_database->select('AUTO_INCREMENT')
            ...
            ->where('TABLE_SCHEMA', $this->_database->database)
            ->get()
            ->row()->AUTO_INCREMENT;
    }
    ...
    /**
     * MySQL DATETIME date_created and date_modified
     */
    public function date_created($row)
    {
        if (is_object($row)) {
    ...
    public function date_modified($row)
    {
        if (is_object($row)) {
    ...
    private function _fetch_primary_key()
    {
        if ($this->primary_key == NULl) {
            $this->primary_key = $this->_database->query(
                "SHOW KEYS FROM `" . $this->_table .
                "` WHERE Key_name = 'PRIMARY'")->row()->Column_name;
        }
    }

    protected function timestamps($data)
    {
        $data['date_created'] = $book['date_modified'] = date('Y-m-d H:i:s');
        return $data;
    }

    protected function userstamps($data)
    {
        $data['created_by'] = $data['modified_by'] = $this->session->userdata('current_user_id');
        return $data;
    }
